...
def load_spectral_data_fixed_headers(eem_file: str,
    ...
    print(f"data loading: EEM={eem_file}, loading={loading_file}, label={label_file}")
    ...
    if len(eem_data) != len(loading_scores) or len(eem_data) != len(labels):
        print(f"warning: the sample sizes are inconsistent")
        eem_data = eem_data[:n]
    ...
    if eem_data.ndim == 2:
        if eem_data.shape[1] == excitation_dim * emission_dim:
            ...
            if not dims:
                raise ValueError(f"Cannot be reshaped into a two-dimensional matrix")
            eem_shape = min(dims, key=lambda x: abs(x[0] - excitation_dim) + abs(x[1] - emission_dim))
            print(f"eem_shape : {eem_shape}")
        eem_data = eem_data.reshape(len(eem_data), *eem_shape)
    else:
        eem_shape = (eem_data.shape[1], eem_data.shape[2])

    print(f"Data loading completed: n={len(eem_data)}, EEM={eem_shape}, loading_dim={loading_scores.shape[1]}, labels={labels.shape[1]}")
...
class BalancedMultiTaskModel(nn.Module):
    def __init__(self,
        ...
        if not 0 <= label4_index < num_labels:
            raise ValueError("label4_index needs to be within the specified quantity range")
    ...
    def step(self, score: float, existence_overfit: bool, regression_underfit: bool, epoch: int) -> Dict[str, float]:
        if score > self.best_score + 1e-4:
            ...
            if self.counter >= self.lr_patience:
                ...
                self.counter = 0
                print(f"The global learning rate coefficient has been reduced to {self.global_factor:.4f}")
...
def train_one_model(model: nn.Module,
    ...
    history: Dict[str, List[float]] = {k: [] for k in [
        ...
        "train_regression_r2_std", "val_regression_r2_std", "train_regression_mse_std", "val_regression_mse_std", 
       "val_final_r2_std", "existence_weight", "regression_weight", "lr_shared", "lr_existence", "lr_regression", "score"
    ]}
    ...
    for epoch in range(num_epochs):
        ...
        tr_pred_arr = np.vstack(tr_pred) if tr_pred else np.zeros((1, 1))
        tr_target_arr = np.vstack(tr_target) if tr_target else np.zeros((1, 1))
        tr_exist_arr = np.vstack(tr_exist) if tr_exist else np.zeros((1, 1))
        train_reg_r2 = calculate_robust_r2(np.vstack(tr_pred), np.vstack(tr_target), np.vstack(tr_exist)) if tr_pred else 0.0
        mask = tr_exist_arr == 1
        train_reg_mse = float(mean_squared_error(tr_target_arr[mask],tr_pred_arr[mask]))
        ...
        history["train_regression_mse_std"].append(train_reg_mse)
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            print(
                        ...
                        f"val_r2={val_metrics['regression_r2_std']:.4f} | "
                        f"val_mse={val_metrics['mse_std']:.4f} | "
                        f"final_r2={val_metrics['final_r2_std']:.4f} | "
                        f"train_r2={train_reg_r2:.4f} | train_mse={train_reg_mse:.4f} | "
                        f"exist_acc={val_metrics['existence_accuracy']:.4f} | "
                        f"lr_reg={lrs.get('regression', 0):.6f} | ew={ew:.3f} rw={rw:.3f}"
                   )
            
        if patience_counter >= patience:
            print(f"Early stop: epoch={epoch + 1}, best_epoch={best_epoch + 1}, best_score={best_score:.4f}")
            break
        if val_metrics["regression_r2_std"] < float(cfg["min_r2_threshold"]) and epoch > 15:
            print(f"Stop early: val regression R2 is too low {val_metrics['regression_r2_std']:.4f}")
            break
...
# 6. Cross-validation, saving, external validation
...
def cross_validate_regression_optimized(eem_data: np.ndarray,
    ...
    if len(REGRESSION_OPTIM_CONFIG["existence_thresholds"]) != num_labels:
        raise ValueError("The length of existence_thresholds must be equal to the numbber labels.")
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(eem_data), start=1):
        ...
        train_loader = DataLoader(train_ds, batch_size=int(REGRESSION_OPTIM_CONFIG["batch_size"]), shuffle=True, num_workers=num_workers, pin_memory=(device.type == "cuda"), persistent_workers= False)
        val_loader = DataLoader(val_ds, batch_size=int(REGRESSION_OPTIM_CONFIG["batch_size"]), shuffle=False, num_workers=num_workers, pin_memory=(device.type == "cuda"), persistent_workers= False)
        ...
        print(f"Fold {fold} completed | score={fold_score:.4f} | label4 ungated R2={metrics.get('ungated_label4_r2', 0.0):.4f} | label4 gated R2={metrics.get('gated_label4_r2', 0.0):.4f} | regression raw ungated R2={metrics.get('regression_r2_raw_ungated', 0.0):.4f} | final raw ungated R2={metrics.get('final_r2_raw_ungated', 0.0):.4f}")
...
def find_latest_model(output_dir: str) -> str:
    ...
    if not candidates:
        raise FileNotFoundError(f"best_model*.pth not found in {output_dir}，please set RUN_CONFIG['model_file']")
...
def external_validate(model_file: str,
    ...
    if tuple(ext_eem_shape) != tuple(eem_shape):
        raise ValueError(f"The external EEM shape {ext_eem_shape} does not match the model training shape {eem_shape}")
    ...
    print(f"External validation prediction has been saved: {output_file}")
    print(f"External validation indicators have been saved: {metrics_path}")
    print("External validation key indicators:")
    print(f"  regression std R2:          {metrics.get('regression_r2_std', 0.0):.4f}")
    print(f"  regression std MSE:         {metrics.get('mse_std', 0.0):.4f}")

    print(f"  regression raw gated R2:    {metrics.get('regression_r2_raw_gated', 0.0):.4f}")
    print(f"  regression raw gated MSE:   {metrics.get('mse_raw_gated', 0.0):.4f}")

    print(f"  regression raw ungated R2:  {metrics.get('regression_r2_raw_ungated', 0.0):.4f}")
    print(f"  regression raw ungated MSE: {metrics.get('mse_raw_ungated', 0.0):.4f}")
    ...
    print(f"  final raw gated MSE:        {metrics.get('final_mse_raw_gated', 0.0):.4f}")
    print(f"  final raw ungated MSE:      {metrics.get('final_mse_raw_ungated', 0.0):.4f}")
    return metrics
...
def main() -> None:
    ...
    if RUN_CONFIG["mode"] == "train":
        ...
        raise ValueError("RUN_CONFIG['mode'] can only be either 'train' or 'external'")
